...
public class DoubleJointedArmSim {
    ...
    private final MechanicalAngularDevice j1Drive;
    private final MechanicalAngularDevice j2Drive;
    ...
    private final Mechanism2d vizMech2d = new Mechanism2d(10, 10);
    private final MechanismRoot2d vizRoot = vizMech2d.getRoot("Root", 30, 30);
    private final MechanismLigament2d vizJ1 =
            vizRoot.append(
                    new MechanismLigament2d(
                            "J1",
                            J1_LENGTH,
                            Units.radiansToDegrees(getJ1Angle()),
                            6,
                            new Color8Bit(Color.kBlue)));
    private final MechanismLigament2d vizJ2 = 
            vizJ1.append(
                    new MechanismLigament2d(
                            "Arm",
                            J2_LENGTH,
                            Units.radiansToDegrees(getJ2Angle()),
                            6,
                            new Color8Bit(Color.kRed)));
    private final DoublePublisher j1AnglePublisher =
            NetworkTableInstance.getDefault().getDoubleTopic("Sim Arm J1 Angle").publish();
    private final DoublePublisher j2AnglePublisher =
            NetworkTableInstance.getDefault().getDoubleTopic("Sim Arm J2 Angle").publish();
    private final DoublePublisher j1VelocityPublisher =
            NetworkTableInstance.getDefault().getDoubleTopic("Sim Arm J1 Velocity").publish();
    private final DoublePublisher j2VelocityPublisher =
            NetworkTableInstance.getDefault().getDoubleTopic("Sim Arm J2 Velocity").publish();
    ...
    public DoubleJointedArmSim(MechanicalAngularDevice j1Drive, MechanicalAngularDevice j2Drive) {
        this.j1Drive = j1Drive;
        this.j2Drive = j2Drive;
        SmartDashboard.putData("Sim Arm", vizMech2d);
    }
    ...
    public void step(double dt) {
        final MechanicalAngularDevice.Action j1Action =
                j1Drive.step(new MechanicalAngularDevice.State(getJ1Angle(), getJ1Velocity()), dt);
        final MechanicalAngularDevice.Action j2Action =
                j2Drive.step(new MechanicalAngularDevice.State(getJ2Angle(), getJ2Velocity()), dt);
        ...
        final var u = VecBuilder.fill(j1Action.torque(), j2Action.torque(), j1Limit, j2Limit);
        final var stateDot = S(state, u);
        state = state.plus(stateDot.times(dt));

        if (publishRate.next()) {
            j1AnglePublisher.set(getJ1Angle());
            j2AnglePublisher.set(getJ2Angle());
            j1VelocityPublisher.set(getJ1Velocity());
            j2VelocityPublisher.set(getJ2Velocity());
            vizJ1.setAngle(Units.radiansToDegrees(getJ1Angle()));
            vizJ2.setAngle(Units.radiansToDegrees(getJ2Angle()));
        }
    }
    ...
    /**
     * Return the angle of the first joint in radians.
     */
    ...
    /**
     * Return the angle of the second joint in radians.
     */
    ...
    /**
     * Return the velocity of the first joint in radians per second.
     */
    ...
    /**
     * Return the velocity of the second joint in radians per second.
     */
