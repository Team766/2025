...
public class DoubleActingPneumaticCylinderSim implements PneumaticDevice, MechanicalDevice {
    ...
    public DoubleActingPneumaticCylinderSim(final double boreDiameter, final double stroke) {
    ...
    public MechanicalDevice.Action step(MechanicalDevice.State state, double dt) {
        final double direction = isExtended ? 1.0 : -1.0;
        return new MechanicalDevice.Action(direction * boreArea() * pneumaticState.pressure());
    }
