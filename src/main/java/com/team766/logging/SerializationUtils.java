...
public class SerializationUtils {
    public static void valueToProto(final Object object, final LogValue.Builder value) {
        if (object instanceof Byte
            ...
            value.setStringValue(ch.toString());
            ...
            value.setFloatValue(num.doubleValue());
        } else if (object instanceof Boolean bool) {
            value.setBoolValue(bool.booleanValue());
        } else if (object instanceof String str) {
            value.setStringValue(str);
        } else if (object == null) {
            value.clearKind();
        } else if (object instanceof Loggable loggable) {
            loggable.toLogValue(value);
        } else {
    ...
    public static Object protoToValue(final LogValue value) {
        switch (value.getKindCase()) {
            ...
            case LIST:
                return value.getList().getElementList().stream()
                        .map(SerializationUtils::protoToValue)
                        .toArray();
            case STRING_VALUE:
        ...
        throw new IllegalArgumentException("Unsupported LogValue kind: " + value.getKindCase());
    }
}
